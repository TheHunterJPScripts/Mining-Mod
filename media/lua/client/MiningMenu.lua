...
MiningMenu.OnFillWorldObjectContextMenu = function(player, context, worldobjects, test)
  ...
  MiningMenu.TableMenuBuilder(context, worldobjects, player, ore)
end
...
MiningMenu.getOreData = function(ore)
  return {
    menuName = getText("ContextMenu_Iron_Ore"),
...
MiningMenu.hasPickaxe = function(player)
  return true
end

MiningMenu.TableMenuBuilder = function(context, worldobjects, player, ore)
  local oreData = MiningMenu.getOreData(ore)
  local menuOption = context:addOption(oreData.menuName, worldobjects,
    MiningMenu.Mine, player, oreData)
  ...
  if MiningMenu.hasPickaxe(player) then
...
MiningMenu.Mine = function(player, oreData)
  if not MiningMenu.hasPickaxe(player) then return end

  local playerObj = getPlayer(player)

  ISTimedActionQueue.add(ISMineOre:new(playerObj, oreData.item, 500));
